# Created with reference to  "Making Games with Python and Pygame" by Al Sweigart
...
minSpeed = 3         # slowest squirrel speed
maxSpeed= 7          # fastest squirrel speed
...
def main():
    ...
    pygame.display.set_caption('Alpha Goose')
    BASICFONT = pygame.font.Font('freesansbold.ttf', 32)

    # load the image files
    L_GOOSE_IMG = pygame.image.load('goose.png')
...
def runGame():
    ...
    winSurf = BASICFONT.render('You have achieved alpha goose status!', True, WHITE)
    ...
    geeseObjs = [] # stores all the non-player squirrel objects
    ...
    while True: # main game loop
        ...
        for sObj in geeseObjs:
            ...
            if random.randint(0, 99) < DIRCHANGEFREQ:
                ...
                if sObj['movex'] > 0: # faces right
                    sObj['surface'] = pygame.transform.scale(R_GOOSE_IMG, (sObj['width'], sObj['height']))
                else: # faces left
                    sObj['surface'] = pygame.transform.scale(L_GOOSE_IMG, (sObj['width'], sObj['height']))
        ...
        for i in range(len(grassObjs) - 1, -1, -1):
            if isOutsideActiveArea(camerax, cameray, grassObjs[i]):
                del grassObjs[i]
        for i in range(len(geeseObjs) - 1, -1, -1):
            if isOutsideActiveArea(camerax, cameray, geeseObjs[i]):
                del geeseObjs[i]

        # add more grass & squirrels if we don't have enough.
        while len(grassObjs) < NUMGRASS:
            grassObjs.append(makeNewGrass(camerax, cameray))
        while len(geeseObjs) < numGeese:
            geeseObjs.append(makeNewGoose(camerax, cameray))
        ...
        for event in pygame.event.get(): # event handling loop
            if event.type == QUIT:
                ...
                # stop moving the player's goose
        ...
        if not gameOverMode:
            ...
            # check if the player has collided with any enemy geese
            for i in range(len(geeseObjs)-1, -1, -1):
                sqObj = geeseObjs[i]
